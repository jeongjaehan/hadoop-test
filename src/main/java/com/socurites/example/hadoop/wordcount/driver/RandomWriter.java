...
implements Tool
{
	public int run(String[] args)
		...
		long numBytesToWritePerMap = job.getLong("test.randomwrite.bytes_per_map", 1073741824L);
	...
	implements Mapper<WritableComparable, Writable, BytesWritable, BytesWritable>
	{
		...
		public void map(WritableComparable key, Writable value, OutputCollector<BytesWritable, BytesWritable> output, Reporter reporter)
			...
			while (this.numBytesToWrite > 0L) {
				...
				reporter.incrCounter(RandomWriter.Counters.BYTES_WRITTEN, keyLength + valueLength);
		...
		{
			this.numBytesToWrite = job.getLong("test.randomwrite.bytes_per_map", 1073741824L);
			...
			this.valueSizeRange = (job.getInt("test.randomwrite.max_value", 20000) - this.minValueSize);
		}
	}
